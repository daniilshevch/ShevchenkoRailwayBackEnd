...
namespace RailwayCore.InternalServices.ExecutiveServices
{
    ...
    public class TicketAllocationService
    {
        ...
        /// <summary>
        /// Даний метод створює квиток(при чому орієнтується на вагон за айді, альтернативний метод - за номером вагона в складі)
        /// </summary>
        ...
        public async Task<QueryResult<TicketBooking>> CreateTicketBooking(InternalTicketBookingDto input)
        {
            ...
            {
                return new FailQuery<TicketBooking>(new Error(ErrorType.NotFound, $"Can't find user with ID: {input.User_Id}", annotation: service_name, unit: ProgramUnit.Core));
            }
    ...
    .FirstOrDefault(carriage_assignment => carriage_assignment.Passenger_Carriage_Id == passenger_carriage_from_ticket.Id);
            ...
            //Проводимо перевірку, чи дане місце є вільним для бронювання. Якщо ні, то помилка
            ...
            {
                return new FailQuery<TicketBooking>(new Error(ErrorType.BadRequest, $"Fail while checking place availability", annotation: service_name, unit: ProgramUnit.Core));
            }
            ...
            if (place_availability == false)
            {
                return new FailQuery<TicketBooking>(new Error(ErrorType.BadRequest, $"Place # {input.Place_In_Carriage} in carriage # {input.Passenger_Carriage_Id} in train route on date {input.Train_Route_On_Date_Id} has  " +
                    $"already been booked", annotation: service_name, unit: ProgramUnit.Core));
            }
            ...
            return new SuccessQuery<TicketBooking>(ticket_booking, new SuccessMessage($"Successfuly created ticket booking. " +
                $"Info:\n{ConsoleLogService.PrintTicketBooking(ticket_booking)}", annotation: service_name, unit: ProgramUnit.Core));
        }
        /// <summary>
        /// Даний метод створює квиток(причому орієнтується на вагон за його номером в складі, альтернативний метод - за айді)
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        [ExecutiveMethod]
        public async Task<QueryResult<TicketBooking>> CreateTicketBookingWithCarriagePositionInSquad(InternalTicketBookingWithCarriagePositionDto input)
        {
            ...
            if (carriage_assignment_from_ticket is null)
            {
                return new FailQuery<TicketBooking>(new Error(ErrorType.BadRequest, $"There is no passenger carriage on position # {input.Passenger_Carriage_Position_In_Squad} in train" +
                    $" route on date # {input.Train_Route_On_Date_Id}", annotation: service_name, unit: ProgramUnit.Core));
            }
            ...
            return new SuccessQuery<TicketBooking>(ticket_booking); //Метод викликає в собі свою перевантажену версію, тому тут нема сенсу прописувати SuccessMessage
        }
    }
}
